...
const render = empInfo => {
    const html = fs.readFileSync(path.resolve(source, "main.html"));
    return newItems(html, "team", empInfo);
}

const newItems = (html, replaced, value) => {
    const items = new RegExp("//" + replaced + "//");
    return html.replace(items, value);
}
